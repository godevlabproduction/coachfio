...
def _swatch(hex_: str) -> Swatch:
    h = hex_.strip().lstrip("#")
    if len(h) != 6:
        raise SystemExit(f"bad colour {hex_!r} - use #RRGGBB")
    return Swatch(int(h[0:2], 16), int(h[2:4], 16), int(h[4:6], 16), 1.0)


def build_theme(art: Path | None, colours: list[str] | None = None) -> dict:
    """Derive a theme from a game's key art, or from colours it was GIVEN.

    Sampling art is the fallback for a game with no published palette. When a
    game has official brand colours, pin them: an average of a promotional
    photograph is a picture of a place, not an identity (the FC art sampled as
    Paris rooftops, not the brand's green).
    """
    if colours:
        sw = [_swatch(c) for c in colours]
        source = "brand palette: " + " ".join(c.upper() for c in colours)
    else:
        sw = swatches(art)
        source = str(art.relative_to(ROOT)).replace("\\", "/")
    if not sw:
...
def write_theme(pkg: Path, game_id: str, edition: str, art: Path | None,
                colours: list[str] | None = None) -> Path:
    theme = build_theme(art, colours)
    theme = {"game_id": game_id, "edition": edition, **theme}
    out = pkg / "config" / "theme.yaml"
    out.parent.mkdir(parents=True, exist_ok=True)
    how = ("pinned from the game's official brand palette"
           if colours else "read from the game's own key art")
    out.write_text(
        f"# GENERATED by tools/extract_palette.py - the game's UI theme,\n"
        f"# {how}. Re-run the tool to change it rather than editing\n"
        "# this file, or the next run silently discards your edits.\n"
...
def main() -> None:
    ...
    ap.add_argument("--art")
    ap.add_argument("--pkg")
    ap.add_argument("--colors", "--colours", dest="colours",
                    help="official palette as #RRGGBB,#RRGGBB,... - used instead of the art")
    ap.add_argument("--all", action="store_true",
                    help="regenerate frontend artifacts from every adapter theme")
    a = ap.parse_args()

    if a.game:
        if not a.pkg or not (a.art or a.colours):
            raise SystemExit("--game needs --pkg and either --art or --colors")
        out = write_theme(ROOT / a.pkg, a.game, a.edition,
                          (ROOT / a.art) if a.art else None,
                          a.colours.split(",") if a.colours else None)
